 ...
 *     注意：这是派生的数据缓存（丢了可后台重扫），不是用户配置，所以域名保持
 *     `token_stats`（storageDomain 只允许小写字母/数字/下划线，且改名会废掉已有
 *     缓存触发全量重扫）；用户偏好（侧栏开关/默认范围）走标准 settings 存储
 *     （settings.yaml: dshp-inx-token-stats，见 lib/index.js），两者按官方分工共存。
...
// 域名保持 token_stats：storageDomain 只允许 [a-z0-9_]（无连字符），且改名会废掉已有缓存。
...
export function createEngine(sessionQuery, dshHome, storageDomain) {
  ...
  const noteStorageError = (label, error) => {
    ...
    if (!storageWarned) {
      storageWarned = true
      try { console.warn('[dshp-inx-token-stats] storage domain ' + storageErr + ' — 缓存持久化不可用，统计仍正常运行（仅重启后重扫）') } catch {}
    }
  }
    ...
    try {
      while (queue.length > 0) {
        ...
        await Promise.all(batch.map(async (job) => {
          try {
            const snap = await withTimeout(sessionQuery.readSession(job.id), READ_TIMEOUT_MS, 'readSession ' + job.id)
