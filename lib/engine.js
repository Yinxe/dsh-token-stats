...
export function createEngine(sessionQuery, dshHome, storageDomain) {
  ...
  /**
   * 降级恢复：存储域曾 open 失败时，每次 snapshot 检查——若仍处降级态
   * 且距上次尝试超过一个泵周期，重试 open（坏文件在首个 put 周期被
   * 全量重写覆盖，schema 修复后自动恢复持久化）。
   */
  async function maybeReopenStorage() {
    if (storageOk) return
    openAttempt++
    if (openAttempt % 5 !== 0) return   // 每 5 次快照尝试一次重开
    try {
      const prev = await tableReady
      if (prev.degraded === true) {
        tableReady = openDomain()
        if (storageOk === false) {
          // 仍失败：保持降级（openDomain 已记录新错误）
        } else {
          storageOk = true
          storageErr = null
        }
